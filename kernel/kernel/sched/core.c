...
#endif
extern void idle_balance_wrr(int dest_cpu, struct rq *target_rq);
int boost_factor = 10;
...
static inline int wrr_prio(struct task_struct *p)
{
...
void sched_fork(struct task_struct *p)
{
	...
	if (p->real_cred->uid >= 10000)
		p->wrr.weight = boost_factor;
	else
		p->wrr.weight = 1;
...
SYSCALL_DEFINE1(get_wrr_info, struct wrr_info*, wrr_info)
{
	int cpu;
	...
	for_each_possible_cpu(cpu) {
		...
		rq = cpu_rq(cpu);
		raw_spin_lock(&rq->lock);
		ret_info.nr_running[cpu] = rq->wrr.wrr_nr_running;
		ret_info.total_weight[cpu] = rq->wrr.total_weights;
		raw_spin_unlock(&rq->lock);
		cpus++;
	}
	ret_info.num_cpus = cpus;
	if (copy_to_user(wrr_info, &ret_info, sizeof(struct wrr_info)))
		return -EFAULT;
	return cpus;
}
